...
select ename "직원이름", dname "부서명", sal||'원' "월급"
from emp e, dept d
...
select job "업무", ename "직원이름",  dname "부서이름"
from emp e, dept d
...
select e.deptno "부서번호",  dname "부서이름", ename "직원이름",  sal "월급", grade "급여등급"
...
--ALLEN              30 SALES         
--WARD               30 SALES         
select nvl(ename,'미정') "이름", d.deptno "부서번호", dname "부서이름"
from emp e, dept d
...
--ALLEN		30	SALES
--ADAMS		0	(null)
select ename "이름", nvl(d.deptno,0) "부서번호", dname "부서이름"
from emp e, dept d
where e.deptno= d.deptno(+);
...
--WARD	30	SALES
--누구?	40	OPERATIONS
select nvl(ename,'누구?') "이름", nvl(to_char(d.deptno),'어디?')"부서번호", nvl(dname,'어디?') "부서이름"
from emp e, dept d
where e.deptno(+)= d.deptno
union
select nvl(ename,'누구?') "이름", nvl(to_char(d.deptno),'어디?')"부서번호", nvl(dname,'어디?') "부서이름"
