...
--9. SALES 부서에서 일하는 직원들의 부서번호, 이름, 직업을 출력한다.
--DEPTNO    ENAME       JOB    			type   
---------- ---------- ---------		-------------
--30번 부서  ALLEN	       SALESMAN		우수사원
--30번 부서  WARD	       SALESMAN		우수사원
--30번 부서  MARTIN      SALESMAN		우수사원
--30번 부서  BLAKE	       MANAGER		우수사원
--30번 부서  TURNER      SALESMAN		우수사원
--30번 부서  JAMES	       CLERK		우수사원
 
 select deptno||'번 부서' "DEPTNO", ename, job, '우수사원' type
 from emp
 where deptno = (select deptno 
                    from dept 
                    where dname='SALES');
